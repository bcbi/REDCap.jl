...
function export_project_info(; url=get_url(), token=get_token(), format=nothing, returnFormat=nothing,)
	#TODO: is this needed?
	if !isnothing(returnFormat); throw(ArgumentError("returnFormat is ignored, so use format instead (possibly this is an issue with REDCap's API")) end
	REDCap.request(;
		url=REDCap_url(url),
