function export_instrument_event_mappings(; url=get_url(), token=get_token(), format=nothing, returnFormat=nothing, arms=nothing,)
	REDCap.request(;
		url=REDCap_url(url),
...
function export_PDF(; url=get_url(), token=get_token(), returnFormat=nothing, record=nothing, event=nothing, instrument=nothing, repeat_instance=nothing, allRecords=nothing, compactDisplay=nothing,)
	REDCap.request(;
		url=REDCap_url(url),
		kwargs = (; token=REDCap_token(token), content=:pdf, record, event, instrument, repeat_instance, allRecords, compactDisplay, returnFormat=REDCap_format(returnFormat),),
	)
end
