...
@testset "Full Functionality" begin
	...
	# Delete: Delete everything from the project piece by piece
	# 	Verify everything, and try to export from it again.
	#
	# Have a file that contains ~4 example records, and all testing data needed? Not include generators? Space vs. complexity
	#
	=#
	if full_test
		println("full test")
	...
	modules = [:(export_field_names(config)),
				...
				:(export_records(config, fields=["record_id","first_name"])), #how many calls to export records? more to test all available fields?
	...
	for m in modules
        #@test eval(m) && true #need way to evaluate these lines as a test...
		try #use carefully!
			eval(m)
			@test true
		catch
    ...
    stock_user_changed=[Dict("username" => "john_smith21@email.com",
        					...
        					"lastname" => "Smithy")]
    response = import_users(config, stock_user_changed)
	@test response == 1
	...
	if full_test
		...
		#Do things to a production project you shouldnt do - eg modifying data locked for in production
